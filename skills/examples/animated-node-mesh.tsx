...
const COLUMNS = 6;
const COL_WIDTH = CANVAS / COLUMNS; // ~86.667
const HALF_COL = COL_WIDTH / 2; // ~43.333
const ROWS = COLUMNS + 1; // 7 rows
const MIN_NODE_R = 1;
const PEAK_NODE_R = 8;
...
/**
 * Node Mesh starter-kit example.
 *
 * Contract:
 * - Keep mesh lines static.
 * - Animate node circle `r` from a low baseline to peak and back.
 * - Twinkle each node independently for a star-like shimmer.
 *
 * See `skills/examples/node-mesh.md` for the parameter contract.
 */
export function AnimatedNodeMeshExample() {
    ...
    () => {
      ...
      if (reduceMotion) {
        nodes.forEach(({ circle }) => {
          circle.setAttribute("r", String(PEAK_NODE_R));
        });
