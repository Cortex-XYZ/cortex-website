...
- `SiteFooter` server component: CTA quote block, newsletter form island, contact mailto, About/Programs/Legal columns, social glyphs (X/Instagram/TikTok/LinkedIn/YouTube), copyright. Module-scope hoisted constants (`QUOTE_LINES`, `TAGLINE_LINES`, `SOCIAL_LINKS`).
...
- Newsletter rate-limit IP handling hardened for Vercel previews: forwarded IP parsing now normalizes IPv4-with-port, bracketed IPv6, and IPv6-mapped IPv4 values; prefers `x-vercel-forwarded-for` before generic proxy headers; supports the standard `Forwarded` header; and skips loopback identifiers in production-like runtimes so deployed requests do not create `cortex:newsletter:127.0.0.1` Upstash buckets. Regression coverage added in `src/lib/newsletter/rate-limit.test.ts`. Verification passed with `bun test src/lib/newsletter/rate-limit.test.ts`, `bun run test`, `bun run typecheck`, `bun run lint` (existing Monad unused-argument warning only), and `bun run build`.
- Newsletter form clears the email input after a successful signup so the success message shows above an empty field ready for another address.
- Upstash unknown-IP fallback fixed: `getNewsletterRateLimitIdentifier` now returns `null` when no client IP can be derived, and `checkNewsletterRateLimit` fails closed in production-like runtimes when the identifier is missing instead of putting all such requests into the shared `local` bucket. Local/dev behavior still skips rate limiting when Upstash env vars are absent and uses the `local` bucket only outside production-like runtimes when a limiter is explicitly configured. Regression coverage added in `src/lib/newsletter/rate-limit.test.ts`. Verification passed with `bun test src/lib/newsletter/rate-limit.test.ts`, `bun run test`, `bun run typecheck`, `bun run lint` (existing Monad unused-argument warning only), and `bun run build`.
- Turnstile hostname validation hardened for deployment: allowed hostnames now support `*.vercel.app` wildcard entries, and the current Vercel deployment hostname from `VERCEL_URL` is merged automatically so preview signups pass Siteverify without listing every preview URL. `.env.example` production guidance updated accordingly. Verification passed with `bun test src/lib/integrations/turnstile.test.ts`, `bun test src/lib/newsletter/submit.test.ts`, `bun test src/lib/integrations/resend.test.ts`, and `bun run typecheck`. Siteverify success responses must now match the newsletter action and an allowed hostname from `TURNSTILE_ALLOWED_HOSTNAMES` (Cloudflare dummy test-secret responses may use the documented `test` action for local testing), `.env.example` includes localhost defaults plus production hostname guidance, and newsletter rate limiting now runs before Siteverify to reduce forged-token validation abuse. Regression coverage added for missing hostname configuration, valid action/hostname responses, action mismatch, hostname mismatch, dummy test action, and rate-limit-before-Turnstile ordering. Verification passed with `bun test src/lib/integrations/turnstile.test.ts`, `bun test src/lib/newsletter/submit.test.ts`, `bun run test`, `bun run typecheck`, `bun run lint` (existing Monad unused-argument warning only), and `bun run build`.
- Turnstile script loader reuse fixed: `NewsletterForm` now stores `loading` / `loaded` / `error` status on the shared Cloudflare script element, uses `window.turnstile.ready()` before setting the client ready state, immediately handles already-loaded scripts on remount, and removes/retries a previously failed script instead of waiting on load/error events that already fired. Verification passed with `bun run typecheck`, `bun run lint` (existing Monad unused-argument warning only), `bun run test`, and `bun run build`.
- Resend re-subscribe behavior fixed: existing contacts are now reactivated with `contacts.update({ unsubscribed: false })` only when Resend reports `unsubscribed: true`; active existing contacts skip the update call. Existing segment members that were just reactivated now return the normal subscription success state instead of the already-submitted state. Regression coverage added for reactivation and the user-facing success message in `src/lib/integrations/resend.test.ts`. Verification passed with `bun test src/lib/integrations/resend.test.ts`, `bun run test`, and `bun run typecheck`.
- Turnstile env sample fixed: `.env.example` now leaves only the localhost always-pass Cloudflare Turnstile test pair active, with production placeholders and the always-fail error-UI pair commented out. This prevents dotenv last-assignment behavior from making copied local env files fail Siteverify by default. Verification: local Next environment-variable docs reviewed; active Turnstile assignments checked with `rg`.
- Newsletter Turnstile hardening complete: invisible Cloudflare Turnstile is wired into the footer newsletter form, Siteverify runs server-side before Upstash rate limiting and Resend, missing/invalid tokens return the short generic verification error without consuming rate-limit quota, and tokens are reset after each server response because Turnstile tokens are single-use. The Turnstile client loader copies an existing page script nonce onto Cloudflare `api.js` so nonce-based CSP can allow Turnstile without making site routes dynamic. Verification passed with `bun run test`, `bun run typecheck`, `bun run lint` (existing Monad warning only), and `bun run build`. Manual localhost test on `http://localhost:3000/#footer` submitted `turnstile-test-1781928453889@example.com` and returned the success message.
